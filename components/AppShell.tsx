    ...
    >
      ...
      >
        <Link href="/daftar-menu">
          <SecondaryMenuItem icon={<FaHome className="text-slate-500 hover:text-brand-600 transition-colors" />} label="Menu" active={pathname === '/daftar-menu'} />
        </Link>
        <div className="w-px h-10 bg-brand-100 mx-2 self-center"></div>
        <Link href="/registrasi">
          <SecondaryMenuItem icon={<FaIdCard className="text-slate-500 hover:text-brand-600 transition-colors" />} label="Registrasi" active={pathname === '/registrasi'} />
        </Link>
        ...
        {!isLoggedIn ? (
          <SecondaryMenuItem
            icon={<FaSignInAlt className="text-slate-500 group-hover:text-brand-600 transition-colors" />}
            label="Log In"
            onClick={() => setIsLoginModalOpen(true)}
          />
        ) : (
          <div className="flex items-center">
            <div className="flex items-center gap-2 px-3 py-1 mr-2 border-r border-brand-200">
        ...
        {contextMenu.show && (
          ...
          >
            <div className="px-4 py-1.5 mb-1 border-b border-brand-50/50 flex items-center gap-2 bg-brand-50/30">
              ...
              <span className="text-[10px] font-extrabold text-brand-700 uppercase tracking-widest">Tindakan Cepat</span>
            </div>
            ...
            <ContextMenuItem icon={<FaSync className="text-brand-500" />} label="Muat Ulang (Refresh)" onClick={() => window.location.reload()} />
            {!isLoggedIn ? (
...
function SecondaryMenuItem({ icon, label, isRed, active, onClick }: { icon: React.ReactNode, label: string, isRed?: boolean, active?: boolean, onClick?: () => void }) {
  return (
    ...
    >
      <span className={`text-2xl drop-shadow-sm z-10 ${active && !isRed ? 'text-brand-600' : ''}`}>{icon}</span>
      <span className={`text-[11px] font-bold z-10 ${isRed ? 'text-red-600' : active ? 'text-brand-700' : 'text-slate-600 group-hover:text-brand-700'}`}>{label}</span>
